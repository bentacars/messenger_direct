...
async function sendImages(psid, urls = []) {
  for (const u of urls) await sendImage(psid, u);
}
    ...
    for (const entry of body.entry || []) for (const ev of entry.messaging || []) {
      ...
      if (state.stage === 'awaiting_selection') {
        ...
        if (Number.isFinite(n) && n>=1 && n<= (state.last?.length||0)) {
          ...
          const imgs = firstFiveImages(row);
          if (imgs.length) await sendImages(psid, imgs);

          await sendText(psid, `🚗 ${cardText(row, state.wants || {})}`);
          await sendQuickReplies(psid, 'Gusto mo bang i-schedule ang viewing?', ['Schedule viewing','More photos','Show other units']);
        ...
        if (/more (photo|photos|pics|pictures|images)/i.test(text)) {
          const more = extraImages(state.lastSelected || {});
